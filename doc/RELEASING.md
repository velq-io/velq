...
Maintainer runbook for shipping Velq across npm, GitHub, and the website-facing changelog surface.
...
Velq uses calendar versions that still fit semver syntax:
...
2. **npm** — `velq` and public workspace packages are published
...
```bash
npx velq@canary onboard
# or
npx velq@canary onboard --data-dir "$(mktemp -d /tmp/velq-canary.XXXXXX)"
```
...
[Run the action here](https://github.com/velq-io/velq/actions/workflows/release.yml)
...
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for Velq. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
```bash
VELQAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```
...
```bash
gh workflow run release-smoke.yml -f velq_version=canary
gh workflow run release-smoke.yml -f velq_version=latest
```

Minimum checks:

- `npx velq@canary onboard` installs
