...
This guide describes the current, implemented way to create a Velq plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Velq app.
...
```bash
pnpm --filter @velq/create-velq-plugin build
node packages/plugins/create-velq-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the Velq repo:

```bash
pnpm --filter @velq/create-velq-plugin build
node packages/plugins/create-velq-plugin/dist/index.js @yourscope/plugin-name \
...
Inside this monorepo, the scaffold uses `workspace:*` for `@velq/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@velq/plugin-sdk` from the local Velq checkout into a `.velq-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into Velq from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@velq/plugin-sdk/ui`
