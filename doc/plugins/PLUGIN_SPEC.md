# Velq Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Velq's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Velq app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Velq HTTP APIs directly.
...
Velq plugin design is based on the following assumptions:

1. Velq is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Velq business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Velq core.
...
2. Let plugins add major capabilities without editing Velq core.
...
4. Depend on project-local plugin folders such as `.velq/plugins`.
...
The single Velq deployment an operator installs and controls.

### 5.2 Company

A first-class Velq business object inside the instance.
...
A trusted in-process extension loaded directly by Velq core.
...
An installable instance-wide extension package loaded through the Velq plugin runtime.
...
Velq has two extension classes.
...
Velq already has a concrete workspace model:
...
Plugins live under the Velq instance directory.

Suggested layout:

- `~/.velq/instances/default/plugins/package.json`
- `~/.velq/instances/default/plugins/node_modules/`
- `~/.velq/instances/default/plugins/.cache/`
- `~/.velq/instances/default/data/plugins/<plugin-id>/`
...
Velq should add CLI commands:

- `pnpm velq plugin list`
- `pnpm velq plugin install <package[@version]>`
- `pnpm velq plugin uninstall <plugin-id>`
- `pnpm velq plugin upgrade <plugin-id> [version]`
- `pnpm velq plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@velq/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface VelqPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that Velq agents can use during runs.
...
- Velq server starts one worker process per installed plugin
...
Receives one typed Velq domain event.
...
export function definePlugin(definition: PluginDefinition): VelqPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: VelqPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@velq/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@velq/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@velq/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@velq/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@velq/plugin-sdk/ui`
...
The `@velq/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Velq secret provider system rather than a plain text input
...
1. Core Velq data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Velq objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Velq product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Velq later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Velq secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm velq plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Velq server. This is a normative requirement, not optional.
...
```ts
import { createTestHarness } from "@velq/plugin-test-harness";
...
For developing a plugin against a running Velq instance:

- The operator installs the plugin from a local path: `pnpm velq plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-velq-plugin`) that scaffolds:

- `package.json` with correct `velqPlugin` keys
...
- `@velq/plugin-workspace-files`
- `@velq/plugin-terminal`
- `@velq/plugin-git`
- `@velq/plugin-linear`
- `@velq/plugin-github-issues`
- `@velq/plugin-grafana`
- `@velq/plugin-runtime-processes`
- `@velq/plugin-stripe`
...
- `@velq/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@velq/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@velq/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@velq/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@velq/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@velq/plugin-sdk/ui`
...
- `@velq/plugin-test-harness`
- `create-velq-plugin` starter template
...
Velq should not implement a generic in-process hook bag modeled directly after local coding tools.

Velq should implement:
...
That is the complete target design for the Velq plugin system.
