...
For `claude_local` and `codex_local`, Velq assumes the CLI is already installed and authenticated on the host machine.
...
Velq stores resumable session state per `(agent, taskKey, adapterType)`.
