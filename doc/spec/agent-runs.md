...
This spec defines how Velq actually runs agents while staying runtime-agnostic.
...
1. Velq is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Velq manages lifecycle, and on exit Velq parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `velq.skill` (shared Velq skill text block)
...
2. Preferred transport is env vars (`VELQ_*`) injected at runtime.
