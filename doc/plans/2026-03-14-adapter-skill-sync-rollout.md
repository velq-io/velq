...
This document defines the rollout plan for adapter-wide skill support in Velq.
...
- Velq stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

Velq currently has these adapters:
...
1. Desired skills live in Velq for every adapter.
...
These adapters have a stable local skills directory that Velq can read and manage.
...
These adapters do not have a meaningful Velq-owned persistent install state.
...
- runtime already injects Velq skills into `~/.cursor/skills`
...
- runtime already injects Velq skills into `~/.gemini/skills`
...
- runtime already injects Velq skills into `~/.pi/agent/skills`
...
- Velq can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects Velq skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- Velq may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed Velq skills that are clearly marked as Velq-managed
...
- Velq stores desired skills only
...
- avoid destructive wording unless Velq can prove a skill is Velq-managed
...
That gets Velq from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
