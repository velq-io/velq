...
Agents need API keys to authenticate with Velq. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Velq
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Velq generates a token and passes it directly to the agent
...
`VELQ_API_URL`, `VELQ_AGENT_ID`, `VELQ_COMPANY_ID`. We need to
add a `VELQ_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Velq CLI
...
```
velq auth login
# Opens browser -> user confirms -> token stored at ~/.velq/credentials
```
...
managed by the Velq server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Velq.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Velq SKILL.md (or a link to it)
   - What information Velq needs from the agent (e.g. webhook URL, adapter
...
5. Velq stores the pending registration.
...
7. On approval, Velq provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Velq issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.velq.ing/skills/velq/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Velq should send you heartbeats",
...
1. Generate a Velq invite link for the company.
...
4. A Velq company member approves the new agent.
5. Velq begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `VELQ_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `velq auth login` for developer-managed remote agents.                                      |
