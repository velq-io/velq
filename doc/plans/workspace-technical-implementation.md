...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/velq-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `VELQ_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `VELQ_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - Velq may create host-local worktrees, processes, and paths
- `cloud`
  - Velq should assume no durable host-local execution workspace management
...
- use `VELQ_EXECUTION_TOPOLOGY` as a defaulting guardrail
