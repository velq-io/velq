...
- New config/env settings:
  - `VELQ_AGENT_JWT_SECRET`
  - `VELQ_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `VELQ_AGENT_JWT_ISSUER` (default: `velq`)
  - `VELQ_AGENT_JWT_AUDIENCE` (default: `velq-api`)
...
1. In:
   ...
   inject `VELQ_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `VELQ_API_KEY`, do not overwrite it.
- Continue injecting:
  - `VELQ_AGENT_ID`
  - `VELQ_COMPANY_ID`
  - `VELQ_API_URL`
...
- Local adapters authenticate without manual `VELQ_API_KEY` config.
