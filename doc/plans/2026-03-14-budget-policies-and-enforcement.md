...
Velq already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model Velq should implement next.

## Product Goals

Velq should let operators:
...
For the next implementation pass, Velq should enforce these defaults:
...
Velq should separate subscription-included usage from billed spend:
...
Velq should have two threshold classes:
...
Velq must know that a project is budget-paused and enforce it.
...
Velq must also block execution before new work starts.
