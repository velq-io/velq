...
Velq's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Velq's API, UI, and data model | Yes |
...
modules/
  observability/
    velq.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@velq/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@velq/core";
...
```
1. Scan modules/ directory for velq.module.json manifests
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@velq/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@velq/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:velq/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.velq.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm velq store list                    # browse available modules and templates
pnpm velq store install <module-id>     # install a module
pnpm velq store import <template-id>    # import a company template
pnpm velq store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Velq instance | `server:started` |
...
4. **Module config** — `velq.config.json` with per-module config, env var interpolation
...
10. **`@velq/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
