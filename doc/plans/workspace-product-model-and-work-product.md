...
Velq needs to support two very different but equally valid ways of working:
...
Today, Velq already has the beginnings of this model:
...
- making Velq feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Velq users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Velq deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Velq into a full code review product
...
This object must be recorded explicitly so that Velq can:
...
Velq should treat PRs as a type of work product linked back to:
...
Velq already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Velq must be a first-class deployment mode

Velq cannot assume that it is running on the same machine as the code.

In cloud deployments, Velq may:
...
- `Runtime service`: a process or service Velq owns or tracks for a workspace
...
It also lets cloud-hosted Velq deployments point at codebases and remotes without pretending the Velq host has direct filesystem access.
...
Without an explicit `execution workspace` record, Velq has nowhere stable to attach:
...
Velq needs a single place to show:
...
- Velq-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Velq owning the execution host
...
## 1. Velq host and execution host must be decoupled
...
without the Velq host itself running local git or local preview processes.
...
Velq should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Velq.
...
Velq should keep workspace automation centered on a small set of hooks:
...
Velq should provide the equivalent visibility in the project `Code` surface:
...
Velq should treat execution workspaces as the natural home for:
...
Velq should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Velq should continue treating cleanup policy as part of the core workspace model:
...
Velq should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Velq host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Velq should borrow Worktrunk's good execution patterns while keeping the broader Velq model:
...
That keeps Velq focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Velq deployments remain first-class

That is a better fit for Velq than either extreme:
