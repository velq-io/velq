...
`PAP-447` asks how Velq should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Velq instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Velq:

- some operators use Velq to manage Velq and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Velq
- Claude and Codex expose different built-in affordances, so Velq should not overfit to one tool

## Core Product Decision

Velq should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Velq
...
- Velq should not assume that every adapter can see or use a host filesystem path directly
...
If Velq makes worktrees agent-first, it will blur:
...
- Velq core resolves and records execution workspace state
...
That gives Velq room to support:
...
- Velq handles workspace runtime services under the hood using project/workspace policy
...
Once Velq is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Velq cannot infer
...
- when issue moves to done, should Velq auto-commit?
- should it auto-push?
...
Velq already has the right foundation for a project-first model.
...
For server-spawned local adapters, Velq already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Velq should mirror directly

Implication:

- for `codex_local`, Velq should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Velq should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Velq should separate:
...
Velq should be able to express intentions such as:
...
These are not specific to Velq. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".velq/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Velq-specific `devServer` flag.
...
Velq should distinguish between:
...
Velq should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Velq should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Velq
...
For runtime services specifically, Velq should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Velq-only dev-server flags
...
3. Inject resolved execution workspace details into `context.velqWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Velq instance can be reused through env/config injection when appropriate
...
- Velq can decide whether to reuse or start a fresh service deterministically
...
3. Test against Velq-on-Velq workflows first.
...
7. Workspace runtime services are modeled generically, not as Velq-specific dev-server toggles.
