...
This document defines the V1 design for a Velq adapter that integrates with
...
4. synthesized stdout events for Velq UI/CLI
...
3. **Agent callback auth to Velq** uses a bootstrap exchange flow (no long-lived Velq key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Velq endpoints, not full SKILL.md prompt injection.
...
- `velqPublicUrl` (optional override; else `VELQ_PUBLIC_URL` env)
...
## Velq Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `VELQ_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.velqPublicUrl`
2. `process.env.VELQ_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Velq credentials in prompt text.

Flow:

1. Before launch/follow-up, Velq mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Velq validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Velq API calls.
...
1. Prompt includes a compact instruction to fetch skills from Velq.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/velq`
   - `GET /api/skills/velq-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <velqPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Velq run context.
...
5. `velqPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Velq skill endpoints |
| Velq auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Velq context.
3. Add richer PR metadata surfacing in Velq UI.
