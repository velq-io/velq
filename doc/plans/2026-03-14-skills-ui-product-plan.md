...
This document defines the product and UI plan for skill management in Velq.
...
- Velq company import/export can include skills as package content
...
3. The UI must always tell the truth about what Velq knows:
   - desired state in Velq
...
Velq should treat skills at two scopes:
...
- created locally inside Velq later
...
7. “Understand what Velq knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Velq-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Velq did not install.
...
- `desired_only`: Velq wants it, adapter does not show it yet
- `external`: adapter has it but Velq does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Velq only tracks desired state
...
- Velq can still track desired skills
...
- `.velq.yaml` may add Velq-specific fidelity, but should not replace the base shortname association model
...
- `.velq.yaml` may add Velq-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Velq should have a company-scoped skill model or managed package model representing:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Velq-specific.
...
That gives Velq one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
