...
This document defines the next-stage plan for Velq company import/export.

The core shift is:

- move from a Velq-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Velq.
...
Velq already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Velq
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `velq.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.velq.yaml` sidecar for high-fidelity Velq-specific details
...
11. adapter-aware skill sync surfaces so Velq can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Velq at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Velq sidecar.
...
Velq must not redefine `SKILL.md`.
...
- Velq-specific fidelity lives in `.velq.yaml`
- Velq may resolve and install `SKILL.md` packages, but it must not require a Velq-only skill format
...
### 5.4 Base Package Vs Velq Extension
...
- base package:
  ...
  - no Velq-only runtime fields by default
- Velq extension:
  - `.velq.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Velq tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`velq.manifest.json` is not part of the future package direction.
...
Velq import/export should support these entity kinds:
...
- Velq can treat “this set of agents came from team package X” as the imported-team model
...
Velq should support source references in package metadata with:
...
Velq should define a new adapter surface area around skills:
...
Planned Velq behavior:

- if an adapter supports read, Velq should show current skills in the UI
- if an adapter supports write, Velq should let the user enable/disable imported skills
- if an adapter supports sync, Velq should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.velq.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.velq.yaml` alongside the base package
...
Velq should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `velq company export <company-id> --out <path>`
- `velq company import <path-or-url> --dry-run`
- `velq company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in Velq storage, or only referenced at import time?
...
3. Should Velq support direct local folder selection in the web UI, or keep that CLI-only initially?
