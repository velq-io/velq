...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Velq plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Velq.
...
- package: `@velq/plugin-kitchen-sink-example`
- plugin id: `velq.kitchen-sink-example` or `velq-kitchen-sink-example`

Recommendation: use `velq-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Velq event such as `issue.created`
...
Recommendation: default to a Velq-local endpoint or a stable public echo endpoint to avoid flaky docs.
