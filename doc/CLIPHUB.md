...
ClipHub is the public registry where people share, discover, and download Velq company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Velq what a package registry is to a programming language. Velq already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Velq — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Velq instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Velq spec. It contains:
...
**Install (fresh start):**
```
velq install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Velq instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Velq, export your company as a template, then publish:

```
velq export --template my-company
velq publish cliphub my-company
```
...
ClipHub is a **separate service** from Velq itself. Velq is self-hosted; ClipHub is a hosted registry that Velq instances talk to.
...
| **Velq CLI** | `velq install`, `velq publish`, `velq cliphub sync` — built into Velq |
| **Velq UI** | "Browse ClipHub" panel in the Velq web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Velq) |
| Backend | TypeScript + Hono (consistent with Velq) |
| Database | PostgreSQL |
...
4. Run `velq install cliphub:acme/lean-saas-shop`
5. Velq creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Velq until it works well
2. Export: `velq export --template my-agency`
3. Publish: `velq publish cliphub my-agency`
...
3. Install just that agent: `velq install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Velq

ClipHub is **not required** to use Velq. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Velq what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `velq install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Velq web UI
- [ ] `velq cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Velq's own philosophy)
