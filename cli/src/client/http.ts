...
export class VelqApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-velq-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  lines.push(
    "",
    "This usually means the Velq server is not running, the configured URL is wrong, or the request is being blocked before it reaches Velq.",
    "",
    "Try:",
    "- Start Velq with `pnpm dev` or `pnpm velq run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Velq is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`VELQ_API_URL\`.`,
  );
  return lines.join("\n");
}
