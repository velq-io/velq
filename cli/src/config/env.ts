...
const JWT_SECRET_ENV_KEY = "VELQ_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by Velq CLI commands",
...
export function resolveVelqEnvFile(configPath?: string): string {
...
export function loadVelqEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeVelqEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readVelqEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeVelqEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeVelqEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readVelqEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeVelqEnvEntries(next, filePath);
  return next;
}
