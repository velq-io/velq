...
import { ApiRequestError, VelqApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Velq config file")
    .option("-d, --data-dir <path>", "Velq data directory root (isolates state from ~/.velq)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Velq API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.VELQ_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.VELQ_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.VELQ_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set VELQ_COMPANY_ID, or set context profile companyId via `velq context set`.",
    );
  }

  const api = new VelqApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.VELQ_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.VELQ_SERVER_PORT || "");
