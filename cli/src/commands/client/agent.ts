...
import type { Agent } from "@velq/shared";
import {
  removeMaintainerOnlySkillSymlinks,
  resolveVelqSkillsDir,
...
function buildAgentEnvExports(input: {
  ...
  return [
    `export VELQ_API_URL='${escaped(input.apiBase)}'`,
    `export VELQ_COMPANY_ID='${escaped(input.companyId)}'`,
    `export VELQ_AGENT_ID='${escaped(input.agentId)}'`,
    `export VELQ_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Velq skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing Velq skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveVelqSkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local Velq skills directory. Expected ./skills in the repo checkout.",
              );
            }
