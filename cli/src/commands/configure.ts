...
function defaultConfig(): VelqConfig {
...
export async function configure(opts: {
  ...
  printVelqCliBanner();
  p.intro(pc.bgCyan(pc.black(" velq configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `velq onboard` first.");
    p.outro("");
    return;
  }

  let config: VelqConfig;
  try {
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because VELQ_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
