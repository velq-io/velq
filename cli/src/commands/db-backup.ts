...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@velq/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://velq:velq@127.0.0.1:${port}/velq`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  printVelqCliBanner();
  p.intro(pc.bgCyan(pc.black(" velq db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveVelqInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "velq";
