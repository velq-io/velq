...
Add first-class **documents** to Velq as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Velq workflow/docs.
...
Update the Velq agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/velq/SKILL.md`
