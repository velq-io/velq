...
Velq companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .velq.yaml     # Adapter config, env inputs, routines
```
...
- **.velq.yaml** holds Velq-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
velq company export <company-id> --out ./my-export
```
...
velq company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
velq company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
velq company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.velq.yaml`
...
```sh
# From a local folder
velq company import ./my-export

# From a GitHub URL
velq company import https://github.com/org/repo

# From a GitHub subfolder
velq company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
velq company import org/repo
velq company import org/repo/companies/acme
```
...
If `--target` is not specified, Velq infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
velq company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
velq company import org/company-templates/engineering-team \
  --target new \
...
```sh
velq company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
velq company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
velq company import ./package \
  --target new \
  --yes \
  --json
```
...
Velq supports several GitHub URL formats:
