...
- Running a Python script that calls the Velq API
...
1. Velq spawns the configured command as a child process
2. Standard Velq environment variables are injected (`VELQ_AGENT_ID`, `VELQ_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the Velq API and perform work.
