...
Build a custom adapter to connect Velq to any agent runtime.
...
2. Build environment with `buildVelqEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `velq run --watch` using `picocolors`.
...
Make Velq skills discoverable to your agent runtime without writing to the agent's working directory:
