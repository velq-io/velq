...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Velq-Signature` + `X-Velq-Timestamp` header pair matching the trigger's signing mode.
