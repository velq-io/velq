# ClipHub: Marketplace for Velq Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Velq configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Velq-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Velq company (agents, projects, governance) |
...
A complete Velq company configuration:
...
Single-agent configurations designed to plug into a Velq org:
...
Portable skill files that any Velq agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Velq's skill loading system
...
interface Listing {
  ...
  velqVersionMin: string;     // Minimum Velq version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Velq company
...
| `POST` | `/api/listings/:id/install` | Install to Velq company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Velq company (or create new)
  → ClipHub API calls Velq API to:
  ...
  → Redirect to Velq dashboard with new team running
```
...
### 5.3 Creator: Export from Velq → Publish

```
Running Velq company → "Export as Blueprint" (CLI or UI)
  → Velq exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Velq brand blue/purple)
...
| Install flow | One-click deploy to Velq company |
...
{
  "targetCompanyId": "uuid",         // Existing Velq company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `velq-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Velq
- **Backend**: Node.js API (or extend Velq server)
- **Database**: Postgres (can share Velq's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Velq auth (or OAuth2)

### 9.2 Integration with Velq

ClipHub can be:
- **Option A**: A separate app that calls Velq's API to install blueprints
- **Option B**: A built-in section of the Velq UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Velq UI and API.
...
- [ ] Install flow: blueprint → Velq company
