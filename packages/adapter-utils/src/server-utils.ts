...
const VELQ_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];

export interface VelqSkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: VelqSkillEntry[];
...
> {
  if (entry.required) {
    return {
      origin: "velq_required",
      originLabel: "Required by Velq",
      readOnly: false,
    };
  }
  return {
    origin: "company_managed",
    originLabel: "Managed by Velq",
    readOnly: false,
  };
}
...
export function buildVelqEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const runtimeHost = resolveHostForUrl(
    process.env.VELQ_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.VELQ_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl = process.env.VELQ_API_URL ?? `http://${runtimeHost}:${runtimePort}`;
  vars.VELQ_API_URL = apiUrl;
  return vars;
}
...
export async function resolveVelqSkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...VELQ_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listVelqSkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<VelqSkillEntry[]> {
  const root = await resolveVelqSkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
...
function normalizeConfiguredVelqRuntimeSkills(value: unknown): VelqSkillEntry[] {
  if (!Array.isArray(value)) return [];
  const out: VelqSkillEntry[] = [];
...
export async function readVelqRuntimeSkillEntries(
  config: Record<string, unknown>,
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<VelqSkillEntry[]> {
  const configuredEntries = normalizeConfiguredVelqRuntimeSkills(config.velqRuntimeSkills);
  if (configuredEntries.length > 0) return configuredEntries;
  return listVelqSkillEntries(moduleDir, additionalCandidates);
}

export async function readVelqSkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
  ...
  const entries = await listVelqSkillEntries(moduleDir);
...
export function readVelqSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw = config.velqSkillSync;
...
function canonicalizeDesiredVelqSkillReference(
  reference: string,
...
export function resolveVelqDesiredSkillNames(
  config: Record<string, unknown>,
  availableEntries: Array<{ key: string; runtimeName?: string | null; required?: boolean }>,
): string[] {
  const preference = readVelqSkillSyncPreference(config);
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredVelqSkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writeVelqSkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
): Record<string, unknown> {
  const next = { ...config };
  const raw = next.velqSkillSync;
  const current =
  ...
  next.velqSkillSync = current;
  return next;
}

export async function ensureVelqSkillSymlink(
  source: string,
  target: string,
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    ...
    // These vars leak in when the Velq server itself is started from
    // within a Claude Code session (e.g. `npx velq run` in a terminal
