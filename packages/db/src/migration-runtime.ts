...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://velq:velq@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "velq");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://velq:velq@127.0.0.1:${preferredPort}/velq`,
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://velq:velq@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "velq");
    return {
      connectionString: `postgres://velq:velq@127.0.0.1:${port}/velq`,
  ...
  const adminConnectionString = `postgres://velq:velq@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "velq");

  return {
    connectionString: `postgres://velq:velq@127.0.0.1:${selectedPort}/velq`,
