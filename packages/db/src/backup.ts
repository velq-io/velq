...
function resolveVelqInstanceId(): string {
  const raw = process.env.VELQ_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveVelqHomeDir(), "instances", resolveVelqInstanceId(), "config.json");
}
...
function resolveDefaultBackupDir(): string {
  return path.resolve(resolveVelqHomeDir(), "instances", resolveVelqInstanceId(), "data", "backups");
}
