...
Example Velq plugin that demonstrates:
...
```bash
pnpm --filter @velq/plugin-file-browser-example build
pnpm velq plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm velq plugin uninstall velq-file-browser-example --force
```
...
- Optional: use `velq-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
