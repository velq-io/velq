...
type VelqSprite = (typeof VELQ_SPRITES)[number];
...
function spriteSize(sprite: VelqSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = VELQ_SPRITES[(baseCol + baseRow) % VELQ_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = VELQ_SPRITES[Math.floor(Math.random() * VELQ_SPRITES.length)]!;
