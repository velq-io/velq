...
Official TypeScript SDK for Velq plugin authors.

- **Worker SDK:** `@velq/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@velq/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@velq/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@velq/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@velq/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@velq/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@velq/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@velq/plugin-sdk/ui/hooks` | Hooks only |
| `@velq/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@velq/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@velq/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@velq/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@velq/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@velq/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main Velq app. They can call ordinary Velq HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Velq instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@velq/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@velq/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Velq information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@velq/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@velq/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@velq/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@velq/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@velq/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@velq/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@velq/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@velq/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@velq/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@velq/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@velq/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@velq/plugin-sdk/bundlers";
...
```bash
velq-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@velq/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__velq__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__velq__/events` streams `reload` SSE events on UI build changes
