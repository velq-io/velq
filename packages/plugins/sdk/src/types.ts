/**
 * Core types for the Velq plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@velq/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @velq/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of Velq object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Velq UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit Velq domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Velq domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the Velq secret provider
...
export interface PluginSecretsClient {
   ...
   * in the Velq secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@velq/plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@velq/plugin-sdk";
 *
