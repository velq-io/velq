 ...
 * channel.
 *
 * The Velq plugin runtime uses JSON-RPC 2.0 over stdio to communicate
...
export type { PluginLauncherRenderContextSnapshot } from "@velq/shared";
 ...
 * we use strings (UUIDs or monotonic counters) for all Velq messages.
 */
...
/**
 * Velq plugin-specific error codes.
 *
...
export interface InitializeParams {
  ...
  instanceInfo: {
    /** UUID of this Velq instance. */
    instanceId: string;
    /** Semver version of the running Velq host. */
    hostVersion: string;
  };
  /** Host API version. */
  apiVersion: number;
