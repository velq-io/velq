/**
 * `@velq/plugin-sdk` — Velq plugin worker-side SDK.
 *
 * This is the main entrypoint for plugin worker code.  For plugin UI bundles,
 * import from `@velq/plugin-sdk/ui` instead.
 *
 * @example
 * ```ts
 * // Plugin worker entrypoint (dist/worker.ts)
 * import { definePlugin, runWorker, z } from "@velq/plugin-sdk";
 *
...
// Manifest and constant types re-exported from @velq/shared
// Plugin authors import manifest types from here so they have a single
// dependency (@velq/plugin-sdk) for all plugin authoring needs.
export type {
