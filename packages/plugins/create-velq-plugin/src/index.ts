...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".velq-sdk");
...
/**
 * Generate a complete Velq plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A Velq plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "velq-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@velq/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@velq/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@velq/plugin-sdk\` and \`@velq/shared\` from a local Velq checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.velq-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-velq-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <velq-sdk-path>]");
    process.exit(1);
  }
