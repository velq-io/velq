...
import type { AdapterModel } from "@velq/adapter-utils";
import { asString, runChildProcess } from "@velq/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.VELQ_PI_COMMAND === "string" &&
    process.env.VELQ_PI_COMMAND.trim().length > 0
      ? process.env.VELQ_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["VELQ_", "npm_", "NPM_"] as const;
