...
## Scope
This plan is now **gateway-only**. Velq supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Velq skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Velq task.
...
```bash
pnpm --filter @velq/server test:run -- openclaw-gateway
pnpm --filter @velq/server typecheck
pnpm --filter @velq/ui typecheck
pnpm --filter velq typecheck
```
