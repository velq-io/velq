...
import { asNumber, asString, buildVelqEnv, parseObject } from "@velq/adapter-utils/server-utils";
...
const DEFAULT_CLIENT_VERSION = "velq";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "velq";
  if (input.strategy === "run") return `velq:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `velq:issue:${input.issueId}`;
  return fallback;
}
...
function resolveVelqApiUrlOverride(value: unknown): string | null {
...
function buildVelqEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const velqApiUrlOverride = resolveVelqApiUrlOverride(ctx.config.velqApiUrl);
  const velqEnv: Record<string, string> = {
    ...buildVelqEnv(ctx.agent),
    VELQ_RUN_ID: ctx.runId,
  };

  if (velqApiUrlOverride) {
    velqEnv.VELQ_API_URL = velqApiUrlOverride;
  }
  if (wakePayload.taskId) velqEnv.VELQ_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) velqEnv.VELQ_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) velqEnv.VELQ_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) velqEnv.VELQ_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) velqEnv.VELQ_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    velqEnv.VELQ_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }

  return velqEnv;
}

function buildWakeText(payload: WakePayload, velqEnv: Record<string, string>): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/velq-claimed-api-key.json";
  const orderedKeys = [
    "VELQ_RUN_ID",
    "VELQ_AGENT_ID",
    "VELQ_COMPANY_ID",
    ...
    "VELQ_TASK_ID",
    "VELQ_WAKE_REASON",
    "VELQ_WAKE_COMMENT_ID",
    "VELQ_APPROVAL_ID",
    "VELQ_APPROVAL_STATUS",
    "VELQ_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = velqEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = velqEnv.VELQ_API_URL ?? "<set VELQ_API_URL>";

  const lines = [
    "Velq wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `VELQ_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load VELQ_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $VELQ_API_KEY on every API call.",
    "- Use X-Velq-Run-Id: $VELQ_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: VELQ_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$VELQ_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    ...
    "   - GET /api/companies/$VELQ_COMPANY_ID/issues?assigneeAgentId=$VELQ_AGENT_ID&status=todo,in_progress,blocked",
...
function buildStandardVelqPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  velqEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateVelq = parseObject(payloadTemplate.velq);
  const workspace = asRecord(ctx.context.velqWorkspace);
  const workspaces = Array.isArray(ctx.context.velqWorkspaces)
    ? ctx.context.velqWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.velqRuntimeServiceIntents)
    ? ctx.context.velqRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardVelq: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: velqEnv.VELQ_API_URL ?? null,
  };

  if (workspace) {
    standardVelq.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardVelq.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardVelq.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "velq-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const velqEnv = buildVelqEnvForWake(ctx, wakePayload);
  const wakeText = buildWakeText(wakePayload, velqEnv);
  ...
  const velqPayload = buildStandardVelqPayload(ctx, wakePayload, velqEnv, payloadTemplate);
