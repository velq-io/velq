...
Use when:
- You want Velq to run Cursor Agent CLI locally as the agent runtime
...
- Velq auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$velq" and related skills on local runs.
- Velq auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
