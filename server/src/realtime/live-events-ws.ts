...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@velq/db";
import type { DeploymentMode } from "@velq/shared";
...
interface IncomingMessageWithContext extends IncomingMessage {
  velqUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).velqUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.velqUpgradeContext = context;
