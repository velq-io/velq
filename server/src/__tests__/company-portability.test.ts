...
import type { CompanyPortabilityFileEntry } from "@velq/shared";
...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "velq",
        name: "velq",
        description: "Velq coordination skill",
        markdown: "---\nname: velq\ndescription: Velq coordination skill\n---\n\n# Velq\n",
        sourceType: "github",
        sourceLocator: "https://github.com/velq-io/velq/tree/master/skills/velq",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/velq/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      owner: "velq",
      repo: "companies",
      ref: "feature/demo",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/velq/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      owner: "velq",
      repo: "companies",
  ...
  it("exports referenced skills as stubs by default with sanitized Velq extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "Velq"');
    ...
    expect(asTextFile(exported.files["agents/claudecoder/AGENTS.md"])).toContain(`- "${velqKey}"`);
    expect(asTextFile(exported.files["agents/cmo/AGENTS.md"])).not.toContain("skills:");
    expect(asTextFile(exported.files["skills/velq/velq/velq/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/velq/velq/velq/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/velq/velq/velq/references/api.md"]).toBeUndefined();
    ...
    const extension = asTextFile(exported.files[".velq.yaml"]);
    expect(extension).toContain('schema: "velq/v1"');
    ...
    expect(extension).not.toContain("velqSkillSync");
  ...
  it("exports default sidebar order into the Velq extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".velq.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/velq/velq/velq/SKILL.md"])).toContain("# Velq");
    expect(asTextFile(exported.files["skills/velq/velq/velq/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/velq/velq/velq/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/velq/velq/velq/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/velq/velq/velq/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .velq.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".velq.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-velq",
        companyId: "company-1",
        key: "velq/velq/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/velq-io/velq/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
        metadata: {
          sourceKind: "velq_bundled",
          owner: "velq",
          repo: "velq",
          ref: "0123456789abcdef0123456789abcdef01234567",
          trackingRef: "master",
    ...
    expect(asTextFile(exported.files["skills/velq/velq/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/velq/velq/release-changelog/SKILL.md"])).toContain("velq/velq/release-changelog");
  });
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/velq",
            repoUrl: "https://github.com/velq-io/velq.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .velq-tmp",
            remoteProvider: null,
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = companyPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "velq-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/velq-io/velq.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
    ...
    const extension = asTextFile(exported.files[".velq.yaml"]);
    expect(extension).toContain('repoUrl: "https://github.com/velq-io/velq.git"');
    expect(extension).toContain('projectWorkspaceKey: "velq"');
  ...
  it("reads env inputs back from .velq.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with Velq routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: "velq-demo", files },
      include: { company: true, agents: true, projects: true, issues: true, skills: false },
      target: { mode: "new_company", newCompanyName: "Imported Velq" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported Velq"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .velq.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported Velq");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".velq.yaml"] = `${exported.files[".velq.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
