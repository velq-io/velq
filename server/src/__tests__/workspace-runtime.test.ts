...
import { resolveVelqConfigPath } from "../paths.ts";
import type { WorkspaceOperation } from "@velq/shared";
...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "velq-worktree-repo-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.email", "velq@example.com"]);
  await runGit(repoRoot, ["config", "user.name", "Velq Test"]);
...
afterEach(async () => {
  ...
  delete process.env.VELQ_WORKTREES_DIR;
...
describe("realizeExecutionWorkspace", () => {
  it("creates and reuses a git worktree for an issue-scoped branch", async () => {
    ...
    expect(first.cwd).toContain(path.join(".velq", "worktrees"));
  ...
  it("runs a configured provision command inside the derived worktree", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$VELQ_WORKSPACE_BRANCH\" > .velq-provision-branch",
        "printf '%s\\n' \"$VELQ_WORKSPACE_BASE_CWD\" > .velq-provision-base",
        "printf '%s\\n' \"$VELQ_WORKSPACE_CREATED\" > .velq-provision-created",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".velq-provision-branch"), "utf8")).resolves.toBe(
      "PAP-448-run-provision-command\n",
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".velq-provision-base"), "utf8")).resolves.toBe(
      `${repoRoot}\n`,
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".velq-provision-created"), "utf8")).resolves.toBe(
      "true\n",
    );
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".velq-provision-created"), "utf8")).resolves.toBe("false\n");
  });

  it("writes an isolated repo-local Velq config and worktree branding when provisioning", async () => {
    const repoRoot = await createTempRepo();
    const previousCwd = process.cwd();
    const velqHome = await fs.mkdtemp(path.join(os.tmpdir(), "velq-worktree-home-"));
    const isolatedWorktreeHome = await fs.mkdtemp(path.join(os.tmpdir(), "velq-worktrees-"));
    const instanceId = "worktree-base";
    const sharedConfigDir = path.join(velqHome, "instances", instanceId);
    const sharedConfigPath = path.join(sharedConfigDir, "config.json");
    const sharedEnvPath = path.join(sharedConfigDir, ".env");

    process.env.VELQ_HOME = velqHome;
    process.env.VELQ_INSTANCE_ID = instanceId;
    process.env.VELQ_WORKTREES_DIR = isolatedWorktreeHome;
    ...
    await fs.writeFile(sharedEnvPath, 'DATABASE_URL="postgres://worktree:test@db.example.com:6543/velq"\n', "utf8");
    ...
    try {
      ...
      const configPath = path.join(workspace.cwd, ".velq", "config.json");
      const envPath = path.join(workspace.cwd, ".velq", ".env");
      ...
      expect(envContents).toContain(`VELQ_HOME=${JSON.stringify(isolatedWorktreeHome)}`);
      expect(envContents).toContain(`VELQ_INSTANCE_ID=${JSON.stringify(expectedInstanceId)}`);
      expect(envContents).toContain(`VELQ_CONFIG=${JSON.stringify(configPath)}`);
      expect(envContents).toContain("VELQ_IN_WORKTREE=true");
      expect(envContents).toContain(
        `VELQ_WORKTREE_NAME=${JSON.stringify("PAP-885-show-worktree-banner")}`,
      );

      process.chdir(workspace.cwd);
      expect(resolveVelqConfigPath()).toBe(configPath);
    } finally {
...
describe("ensureRuntimeServicesForRun", () => {
  it("reuses shared runtime services across runs and starts a new service after release", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "velq-runtime-workspace-"));
  ...
  it("does not leak parent Velq instance env into runtime service commands", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "velq-runtime-env-"));
    const workspace = buildWorkspace(workspaceRoot);
    ...
    const serviceCommand = [
      ...
      JSON.stringify(
        [
          ...
          "velqConfig: process.env.VELQ_CONFIG ?? null,",
          "velqHome: process.env.VELQ_HOME ?? null,",
          "velqInstanceId: process.env.VELQ_INSTANCE_ID ?? null,",
    ...
    process.env.VELQ_CONFIG = "/tmp/base-velq-config.json";
    process.env.VELQ_HOME = "/tmp/base-velq-home";
    process.env.VELQ_INSTANCE_ID = "base-instance";
    process.env.DATABASE_URL = "postgres://shared-db.example.com/velq";
    ...
    expect(captured.velqConfig).toBeNull();
    expect(captured.velqHome).toBeNull();
    expect(captured.velqInstanceId).toBeNull();
  ...
  it("stops execution workspace runtime services by executionWorkspaceId", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "velq-runtime-stop-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("does not stop services in sibling directories when matching by workspace cwd", async () => {
    const workspaceParent = await fs.mkdtemp(path.join(os.tmpdir(), "velq-runtime-sibling-"));
