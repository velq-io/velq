...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Velq OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested Velq base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("velqApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.velqApiUrl");
    expect(text).toContain("~/.openclaw/workspace/velq-claimed-api-key.json");
    expect(text).toContain("VELQ_API_KEY");
