...
import type { VelqConfig } from "@velq/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by Velq worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.VELQ_IN_WORKTREE !== "true") return null;

  const configPath = resolveVelqConfigPath(overrideConfigPath);
  const envPath = resolveVelqEnvPath(configPath);
  const worktreeRoot = path.resolve(path.dirname(configPath), "..");
  const worktreeName = nonEmpty(env.VELQ_WORKTREE_NAME) ?? path.basename(worktreeRoot);
  const instanceId = nonEmpty(env.VELQ_INSTANCE_ID) ?? sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(env.VELQ_HOME) ??
      nonEmpty(env.VELQ_WORKTREES_DIR) ??
      "~/.velq-worktrees",
  );
...
function writeConfigFile(configPath: string, config: VelqConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".velq", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as VelqConfig;
...
): { config: VelqConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.VELQ_HOME = context.homeDir;
  process.env.VELQ_INSTANCE_ID = context.instanceId;
  process.env.VELQ_CONFIG = context.configPath;
  process.env.VELQ_CONTEXT = context.contextPath;
  process.env.VELQ_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as VelqConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    VELQ_HOME: context.homeDir,
    VELQ_INSTANCE_ID: context.instanceId,
    VELQ_CONFIG: context.configPath,
    VELQ_CONTEXT: context.contextPath,
    VELQ_IN_WORKTREE: "true",
    VELQ_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: VelqConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as VelqConfig;
  } catch {
    return;
  }
