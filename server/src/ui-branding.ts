const FAVICON_BLOCK_START = "<!-- VELQ_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- VELQ_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- VELQ_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- VELQ_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.VELQ_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.VELQ_WORKTREE_NAME) ?? nonEmpty(env.VELQ_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.VELQ_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="velq-worktree-enabled" content="true" />',
    `<meta name="velq-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="velq-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="velq-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
