...
import type { AdapterRuntimeServiceReport } from "@velq/adapter-utils";
import type { Db } from "@velq/db";
import { workspaceRuntimeServices } from "@velq/db";
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.VELQ_WORKSPACE_CWD = input.worktreePath;
  env.VELQ_WORKSPACE_PATH = input.worktreePath;
  env.VELQ_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.VELQ_WORKSPACE_BRANCH = input.branchName;
  env.VELQ_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.VELQ_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.VELQ_WORKSPACE_SOURCE = input.base.source;
  env.VELQ_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.VELQ_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.VELQ_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.VELQ_PROJECT_ID = input.base.projectId ?? "";
  env.VELQ_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.VELQ_AGENT_ID = input.agent.id;
  env.VELQ_AGENT_NAME = input.agent.name;
  env.VELQ_COMPANY_ID = input.agent.companyId;
  env.VELQ_ISSUE_ID = input.issue?.id ?? "";
  env.VELQ_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.VELQ_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.VELQ_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.VELQ_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.VELQ_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.VELQ_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.VELQ_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.VELQ_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.VELQ_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.VELQ_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.VELQ_PROJECT_ID = input.workspace.projectId ?? "";
  env.VELQ_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.VELQ_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".velq", "worktrees");
