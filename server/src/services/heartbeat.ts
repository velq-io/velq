...
import type { BillingType } from "@velq/shared";
import {
  agents,
  agentRuntimeState,
  agentTaskSessions,
...
const DEFERRED_WAKE_CONTEXT_KEY = "_velqWakeContext";
const DETACHED_PROCESS_ERROR_CODE = "process_detached";
const startLocksByAgent = new Map<string, Promise<void>>();
const REPO_ONLY_CWD_SENTINEL = "/__velq_repo_only__";
...
export function heartbeatService(db: Db) {
  ...
  async function executeRun(runId: string) {
    ...
    const runtimeConfig = {
      ...resolvedConfig,
      velqRuntimeSkills: runtimeSkillEntries,
    };
    ...
    context.velqWorkspaces = resolvedWorkspace.workspaceHints;
    ...
    if (runtimeServiceIntents.length > 0) {
      context.velqRuntimeServiceIntents = runtimeServiceIntents;
    } else {
      delete context.velqRuntimeServiceIntents;
    }
    ...
    if (sessionCompaction.rotate) {
      context.velqSessionHandoffMarkdown = sessionCompaction.handoffMarkdown;
      context.velqSessionRotationReason = sessionCompaction.reason;
      context.velqPreviousSessionId = previousSessionDisplayId ?? runtimeSessionIdForAdapter;
      ...
      delete context.velqSessionHandoffMarkdown;
      delete context.velqSessionRotationReason;
      delete context.velqPreviousSessionId;
    }
    ...
    try {
      ...
      if (runtimeServices.length > 0) {
        context.velqRuntimeServices = runtimeServices;
        context.velqRuntimePrimaryUrl =
      ...
      if (issueId && (executionWorkspace.created || runtimeServices.some((service) => !service.reused))) {
        try {
          ...
          await onLog(
            "stderr",
            `[velq] Failed to post workspace-ready comment: ${err instanceof Error ? err.message : String(err)}\n`,
          );
        }
      }
      ...
      if (adapter.supportsLocalAgentJwt && !authToken) {
        logger.warn(
          ...
          "local agent jwt secret missing or invalid; running without injected VELQ_API_KEY",
        );
      }
      ...
      if (adapterManagedRuntimeServices.length > 0) {
        ...
        context.velqRuntimeServices = combinedRuntimeServices;
        context.velqRuntimePrimaryUrl =
        ...
        if (issueId) {
          try {
            ...
            await onLog(
              "stderr",
              `[velq] Failed to post adapter-managed runtime comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
      }
