 ...
 */
import { pluginManifestV1Schema } from "@velq/shared";
import type { VelqPluginManifestV1 } from "@velq/shared";
import { PLUGIN_API_VERSION } from "@velq/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): VelqPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as VelqPluginManifestV1,
        };
      }
