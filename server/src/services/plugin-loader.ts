 ...
 *    (`~/.velq/plugins/`) and `node_modules` for packages matching
 *    the `velq-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Velq plugins.
 * Packages matching this pattern are considered Velq plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "velq-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: VelqPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.velq/plugins/ local directory
  | "npm"               // npm packages matching velq-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: VelqPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the velq-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "velq-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: VelqPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover Velq plugins installed as npm packages in the current
   * Node.js environment matching the "velq-plugin-*" naming convention.
   *
   ...
   * the "velqPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Velq plugin.
   * Throws if the package is a Velq plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<VelqPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: VelqPluginManifestV1;
    newManifest: VelqPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the Velq plugin naming convention.
 * Accepts both the "velq-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @velq/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "velqPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const velqPlugin = pkgJson["velqPlugin"];
  if (
    velqPlugin !== null &&
    typeof velqPlugin === "object" &&
    !Array.isArray(velqPlugin)
  ) {
    const manifestRelPath = (velqPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: VelqPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumVelqVersion;
}
 ...
 *   packageName: "velq-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: VelqPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as VelqPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Velq plugin (no manifest found).`,
      );
    }
   ...
   * if the package is not a Velq plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasVelqPlugin = "velqPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasVelqPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<VelqPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: VelqPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @velq/shared exports). Run those workers through
