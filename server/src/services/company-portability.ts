...
import {
  readVelqSkillSyncPreference,
  writeVelqSkillSyncPreference,
...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".velq.yaml" || normalized === ".velq.yml") return "extension";
...
function readSkillKey(frontmatter: Record<string, unknown>) {
  ...
  const velq = isPlainRecord(metadata?.velq) ? metadata?.velq as Record<string, unknown> : null;
  return normalizeSkillKey(
    asString(frontmatter.key)
    ?? asString(frontmatter.skillKey)
    ?? asString(metadata?.skillKey)
    ?? asString(metadata?.canonicalKey)
    ?? asString(metadata?.velqSkillKey)
    ?? asString(velq?.skillKey)
    ?? asString(velq?.key),
  );
}
...
function deriveManifestSkillKey(
  ...
  if (sourceKind === "velq_bundled") {
    return `velq/velq/${slug}`;
  }
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .velq.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .velq.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .velq.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .velq.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }

  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; Velq will import the routine trigger without those limits.`);
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .velq.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .velq.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .velq.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .velq.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .velq.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .velq.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .velq.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .velq.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .velq.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function filterExportFiles(
  ...
  const extensionEntry = filtered[velqExtensionPath];
  if (selectedFiles.has(velqExtensionPath) && typeof extensionEntry === "string") {
    filtered[velqExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }

  return filtered;
}

function findVelqExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".velq.yaml"] === "string") return ".velq.yaml";
  if (typeof files[".velq.yml"] === "string") return ".velq.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.velq.yaml") || entry.endsWith("/.velq.yml")) ?? null;
}
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "velq_bundled") {
    ...
    return {
      ...
      commit,
      trackingRef: "master",
      url: `https://github.com/velq-io/velq/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "velq_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.velqSkillKey = skill.key;
  metadata.velq = {
    ...(isPlainRecord(metadata.velq) ? metadata.velq : {}),
    skillKey: skill.key,
    slug: skill.slug,
  };
  const frontmatter = {
...
function buildManifestFromPackageFiles(
  ...
  const velqExtensionPath = findVelqExtensionPath(normalizedFiles);
  const velqExtension = velqExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, velqExtensionPath) ?? "")
    : {};
  const velqCompany = isPlainRecord(velqExtension.company) ? velqExtension.company : {};
  const velqSidebar = normalizePortableSidebarOrder(velqExtension.sidebar);
  const velqAgents = isPlainRecord(velqExtension.agents) ? velqExtension.agents : {};
  const velqProjects = isPlainRecord(velqExtension.projects) ? velqExtension.projects : {};
  const velqTasks = isPlainRecord(velqExtension.tasks) ? velqExtension.tasks : {};
  const velqRoutines = isPlainRecord(velqExtension.routines) ? velqExtension.routines : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      brandColor: asString(velqCompany.brandColor),
      logoPath: asString(velqCompany.logoPath) ?? asString(velqCompany.logo),
      requireBoardApprovalForNewAgents:
        typeof velqCompany.requireBoardApprovalForNewAgents === "boolean"
          ? velqCompany.requireBoardApprovalForNewAgents
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(velqAgents[slug]) ? velqAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(velqProjects[slug]) ? velqProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(velqTasks[slug]) ? velqTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(velqRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(velqRoutines[slug]) ? velqRoutines[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
  ...
  async function exportBundle(
    ...
    const velqAgentsOut: Record<string, Record<string, unknown>> = {};
    const velqProjectsOut: Record<string, Record<string, unknown>> = {};
    const velqTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const velqRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readVelqSkillSyncPreference(
        ...
        velqAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      velqProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      velqTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      velqRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const velqExtensionPath = ".velq.yaml";
    const velqAgents = Object.fromEntries(
      Object.entries(velqAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const velqProjects = Object.fromEntries(
      Object.entries(velqProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const velqTasks = Object.fromEntries(
      Object.entries(velqTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const velqRoutines = Object.fromEntries(
      Object.entries(velqRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[velqExtensionPath] = buildYamlFile(
      {
        schema: "velq/v1",
        ...
        agents: Object.keys(velqAgents).length > 0 ? velqAgents : undefined,
        projects: Object.keys(velqProjects).length > 0 ? velqProjects : undefined,
        tasks: Object.keys(velqTasks).length > 0 ? velqTasks : undefined,
        routines: Object.keys(velqRoutines).length > 0 ? velqRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, velqExtensionPath);
  ...
  async function importBundle(
    ...
    if (include.agents) {
      for (const planAgent of plan.preview.plan.agentPlans) {
        ...
        const adapterConfigWithSkills = writeVelqSkillSyncPreference(
          baseAdapterConfig,
          desiredSkills,
        );
