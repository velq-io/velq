...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@velq/db";
 ...
 *
 * This implementation delegates to the core Velq domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@velq/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
