...
import type { DeploymentExposure, DeploymentMode } from "@velq/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "velq" &&
    normalized !== "velq-create-agent" &&
    normalized !== "velq-create-plugin" &&
...
/** Resolve the Velq repo skills directory (built-in / managed skills). */
function resolveVelqSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const velqSkillsDir = resolveVelqSkillsDir();

  // Build set of Velq-managed skill names
  const velqSkillNames = new Set<string>();
  if (velqSkillsDir) {
    try {
      for (const entry of fs.readdirSync(velqSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) velqSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isVelqManaged: velqSkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.velqApiUrl)) {
    const legacyVelqApiUrl = nonEmptyTrimmedString(input.velqApiUrl);
    if (legacyVelqApiUrl) merged.velqApiUrl = legacyVelqApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    velqApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.velqApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawVelqApiUrl =
    typeof defaults.velqApiUrl === "string"
      ? defaults.velqApiUrl.trim()
      : "";
  if (rawVelqApiUrl) {
    try {
      const parsedVelqApiUrl = new URL(rawVelqApiUrl);
      if (
        parsedVelqApiUrl.protocol !== "http:" &&
        parsedVelqApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_velq_api_url_protocol",
          level: "warn",
          message: `velqApiUrl must use http:// or https:// (got ${parsedVelqApiUrl.protocol}).`
        });
      } else {
        normalized.velqApiUrl = parsedVelqApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_velq_api_url_configured",
          level: "info",
          message: `velqApiUrl set to ${parsedVelqApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_velq_api_url_invalid",
        level: "warn",
        message: `Invalid velqApiUrl: ${rawVelqApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Velq host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Velq is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm velq allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/velq";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/velq-claimed-api-key.json and load VELQ_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: velqApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Velq hostname is reachable and allowed via `pnpm velq allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Velq API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/velq/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # Velq OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          velqApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, Velq generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "velqApiUrl": "https://velq-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in Velq before key claim is allowed.
    ...
    - ~/.openclaw/workspace/velq-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/velq-claimed-api-key.json

    And set the VELQ_API_KEY and VELQ_API_URL in your environment variables as specified here:
    ...
    Then set VELQ_API_KEY and VELQ_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install Velq skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your VELQ_API_URL to the top of your skill and note the path to your VELQ_API_URL
    ...
    ${
      ...
      "Ensure Velq is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Velq base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.velqApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm velq allowed-hostname <host>
      - then restart Velq and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@velq.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "velq", path: "/api/skills/velq" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "velq-create-agent",
          path: "/api/skills/velq-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              velqApiUrl: req.body.velqApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.velqApiUrl &&
          !persistedDefaults.velqApiUrl
        ) {
          missingPersistedFields.push("velqApiUrl");
        }
