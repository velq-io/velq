...
import { agents as agentsTable, companies, heartbeatRuns } from "@velq/db";
...
import { runClaudeLogin } from "@velq/adapter-claude-local/server";
import {
  DEFAULT_CODEX_LOCAL_BYPASS_APPROVALS_AND_SANDBOX,
  DEFAULT_CODEX_LOCAL_MODEL,
} from "@velq/adapter-codex-local";
import { DEFAULT_CURSOR_LOCAL_MODEL } from "@velq/adapter-cursor-local";
import { DEFAULT_GEMINI_LOCAL_MODEL } from "@velq/adapter-gemini-local";
import { ensureOpenCodeModelConfiguredAndAvailable } from "@velq/adapter-opencode-local/server";
import {
...
export function agentRoutes(db: Db) {
  ...
  const strictSecretsMode = process.env.VELQ_SECRETS_STRICT_MODE === "true";
  ...
  async function resolveDesiredSkillAssignment(
    ...
    return {
      adapterConfig: writeVelqSkillSyncPreference(adapterConfig, desiredSkills),
      desiredSkills,
      runtimeSkillEntries,
    };
  }
  ...
  router.get("/agents/:id/skills", async (req, res) => {
    ...
    if (!adapter?.listSkills) {
      const preference = readVelqSkillSyncPreference(
