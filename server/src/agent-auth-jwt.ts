...
function jwtConfig() {
  const secret = process.env.VELQ_AGENT_JWT_SECRET;
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.VELQ_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.VELQ_AGENT_JWT_ISSUER ?? "velq",
    audience: process.env.VELQ_AGENT_JWT_AUDIENCE ?? "velq-api",
  };
}
