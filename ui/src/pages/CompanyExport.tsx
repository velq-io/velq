...
/**
 * Filter .velq.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterVelqYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://velq.ing) package.");
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .velq.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .velq.yaml
    const yamlPath = exportData.velqExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterVelqYaml(exportData.files[yamlPath], checkedFiles);
    }
