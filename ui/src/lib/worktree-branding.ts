...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("velq-worktree-enabled") !== "true") return null;

  const name = readMetaContent("velq-worktree-name");
  const color = normalizeHexColor(readMetaContent("velq-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("velq-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
