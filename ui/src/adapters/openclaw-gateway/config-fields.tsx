...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Velq API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "velqApiUrl",
                  String(config.velqApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "velqApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://velq.example"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "velq"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. Velq persists a device key during onboarding so pairing approvals
