...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`velq-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="velq-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("velq-mermaid-status", error && "velq-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="velq-mermaid-source">
...
export function MarkdownBody({ children, className, resolveImageSrc }: MarkdownBodyProps) {
      ...
      if (parsed) {
        ...
        return (
          <a
            ...
            className={cn(
              "velq-mention-chip",
              `velq-mention-chip--${parsed.kind}`,
              parsed.kind === "project" && "velq-project-mention-chip",
            )}
  ...
  return (
    <div
      className={cn(
        "velq-markdown prose prose-sm max-w-none break-words overflow-hidden",
