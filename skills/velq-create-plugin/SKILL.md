---
name: velq-create-plugin
description: >
  Create new Velq plugins with the current alpha SDK/runtime. Use when
...
# Create a Velq Plugin

Use this skill when the task is to create, scaffold, or document a Velq plugin.
...
node packages/plugins/create-velq-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the Velq repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.velq-sdk/`:

```bash
pnpm --filter @velq/create-velq-plugin build
node packages/plugins/create-velq-plugin/dist/index.js @acme/plugin-name \
...
- is installed into Velq from an absolute local path during development
