...
VELQ_HOST_PORT="${VELQ_HOST_PORT:-3100}"
VELQ_HOST_FROM_CONTAINER="${VELQ_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.velq-openclaw.override.yml"
...
detect_velq_base_url() {
  ...
  for candidate in "$VELQ_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${VELQ_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${VELQ_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
velq_base_url="$(detect_velq_base_url || true)"
...
Velq URL for OpenClaw container:
EOF
  if [[ -n "$velq_base_url" ]]; then
    cat <<EOF
  $velq_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${VELQ_HOST_PORT}
  (Do not use http://127.0.0.1:${VELQ_HOST_PORT} inside the container.)
  If Velq rejects the host, run on host machine:
    pnpm velq allowed-hostname host.docker.internal
  Then restart Velq and re-run this script.
EOF
  fi
  cat <<EOF
