...
const devServerStatusFilePath = path.join(repoRoot, ".velq", "dev-server-status.json");
...
const env = {
  ...process.env,
  VELQ_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "dev") {
  env.VELQ_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
}

if (mode === "watch") {
  env.VELQ_MIGRATION_PROMPT ??= "never";
  env.VELQ_MIGRATION_AUTO_APPLY ??= "true";
}

if (tailscaleAuth) {
  env.VELQ_DEPLOYMENT_MODE = "authenticated";
  env.VELQ_DEPLOYMENT_EXPOSURE = "private";
  env.VELQ_AUTH_BASE_URL_MODE = "auto";
  env.HOST = "0.0.0.0";
  console.log("[velq] dev mode: authenticated/private (tailscale-friendly) on 0.0.0.0");
} else {
  console.log("[velq] dev mode: local_trusted (default)");
}
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@velq/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[velq] Command failed with code ${status.code}: pnpm --filter @velq/db exec tsx src/migration-status.ts --json\n`,
    );
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[velq] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options = {}) {
  ...
  const autoApply = options.autoApply ?? env.VELQ_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[velq] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). ` +
...
async function buildPluginSdk() {
  console.log("[velq] building plugin sdk...");
  const result = await runPnpm(
    ["--filter", "@velq/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
  ...
  if (result.code !== 0) {
    console.error("[velq] plugin sdk build failed");
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@velq/server", serverScript, ...forwardedArgs],
