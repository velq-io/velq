...
IMAGE_NAME="${IMAGE_NAME:-velq-onboard-smoke}"
HOST_PORT="${HOST_PORT:-3131}"
VELQAI_VERSION="${VELQAI_VERSION:-latest}"
...
VELQ_DEPLOYMENT_MODE="${VELQ_DEPLOYMENT_MODE:-authenticated}"
VELQ_DEPLOYMENT_EXPOSURE="${VELQ_DEPLOYMENT_EXPOSURE:-private}"
VELQ_PUBLIC_URL="${VELQ_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
SMOKE_AUTO_BOOTSTRAP="${SMOKE_AUTO_BOOTSTRAP:-true}"
SMOKE_ADMIN_NAME="${SMOKE_ADMIN_NAME:-Smoke Admin}"
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@velq.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-velq-smoke-password}"
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e VELQ_DEPLOYMENT_MODE="$VELQ_DEPLOYMENT_MODE" \
      -e VELQ_DEPLOYMENT_EXPOSURE="$VELQ_DEPLOYMENT_EXPOSURE" \
      -e VELQ_PUBLIC_URL="$VELQ_PUBLIC_URL" \
      -e VELQ_HOME="/velq" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "velq@${VELQAI_VERSION}" auth bootstrap-ceo --data-dir "$VELQ_HOME" --base-url "$VELQ_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$VELQ_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$VELQ_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$VELQ_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$VELQ_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$VELQ_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$VELQ_PUBLIC_URL/api/companies")"
...
docker build \
  --build-arg VELQAI_VERSION="$VELQAI_VERSION" \
...
echo "    Public URL: $VELQ_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Detached mode: $SMOKE_DETACH"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $VELQ_DEPLOYMENT_MODE/$VELQ_DEPLOYMENT_EXPOSURE"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/velq-onboard-smoke.XXXXXX")"
COOKIE_JAR="$TMP_DIR/cookies.txt"

if ! wait_for_http "$VELQ_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $VELQ_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$VELQ_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $VELQ_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"
