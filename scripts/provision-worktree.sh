...
base_cwd="${VELQ_WORKSPACE_BASE_CWD:?VELQ_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${VELQ_WORKSPACE_CWD:?VELQ_WORKSPACE_CWD is required}"
velq_home="${VELQ_HOME:-$HOME/.velq}"
velq_instance_id="${VELQ_INSTANCE_ID:-default}"
velq_dir="$worktree_cwd/.velq"
worktree_config_path="$velq_dir/config.json"
worktree_env_path="$velq_dir/.env"
worktree_name="${VELQ_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
source_config_path="${VELQ_CONFIG:-}"
if [[ -z "$source_config_path" && ( -e "$base_cwd/.velq/config.json" || -L "$base_cwd/.velq/config.json" ) ]]; then
  source_config_path="$base_cwd/.velq/config.json"
fi
if [[ -z "$source_config_path" ]]; then
  source_config_path="$velq_home/instances/$velq_instance_id/config.json"
fi
...
run_isolated_worktree_init() {
  if command -v pnpm >/dev/null 2>&1 && pnpm velq --help >/dev/null 2>&1; then
    pnpm velq worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    return 0
  fi

  if command -v velq >/dev/null 2>&1; then
    velq worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    return 0
  fi
...
write_fallback_worktree_config() {
  ...
  VELQ_WORKTREES_DIR="${VELQ_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const velqDir = process.env.VELQ_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.VELQ_WORKTREES_DIR) ?? "~/.velq-worktrees"));
  const instanceId = sanitizeInstanceId(worktreeName);
  const instanceRoot = path.resolve(worktreeHome, "instances", instanceId);
  const configPath = path.resolve(velqDir, "config.json");
  const envPath = path.resolve(velqDir, ".env");
  ...
  const targetConfig = {
    ...
    storage: {
      ...
      s3: {
        bucket: sourceConfig?.storage?.s3?.bucket ?? "velq",
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.VELQ_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.VELQ_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.VELQ_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "VELQ_HOME=" + JSON.stringify(worktreeHome),
    "VELQ_INSTANCE_ID=" + JSON.stringify(instanceId),
    "VELQ_CONFIG=" + JSON.stringify(configPath),
    "VELQ_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "VELQ_IN_WORKTREE=true",
    "VELQ_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.VELQ_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("VELQ_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
...
if ! run_isolated_worktree_init; then
  echo "velq CLI not available in this workspace; writing isolated fallback config without DB seeding." >&2
  write_fallback_worktree_config
fi
